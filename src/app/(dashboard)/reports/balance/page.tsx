...
export default function BalanceReportPage() {
  ...
  const euroRateDetail = useMemo(
    () => balanceRows.find((row) => row.currency === "EUR") ?? null,
    [],
  );
  ...
  const handleDownload = () => {
    ...
    setDownloading(true);
    try {
      const exportRows = balanceRows.map((row) => ({
        Producto: row.productName,
        Nombre: row.name,
        Moneda: row.currency,
        Inicial: row.initial,
        Movimientos: row.transact,
        Final: row.final,
        "Actual USD": row.actualUsd,
        "Compra %": row.buyPct,
        "Venta %": row.sellPct,
        "Tasa compra": row.buyRate,
        "Tasa venta": row.sellRate,
        Notas: row.notes,
      }));
      ...
      anchor.href = url;
      anchor.download = `balance-${new Date().toISOString().slice(0, 10)}.xlsx`;
      anchor.click();
        ...
        <div className="flex gap-3 pb-2">
          {balanceCards.map((card) => {
            ...
            return (
              <div
                key={card.code}
                className="min-w-[200px] rounded-xl border border-gray-200 bg-white p-4 shadow-sm"
              >
                <div className="text-xs text-gray-500">{card.name}</div>
                <div className="mt-1 text-lg font-semibold text-gray-900">
                  {formatNumber(card.balance)} {card.code}
                </div>
                <div
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 px-4 py-3">
          <div>
            ...
            <p className="text-xs text-gray-500">
              Producto = moneda. Tabla lista para agregar filas.
            </p>
          </div>
                ...
                <tr key={row.productName} className="border-b border-gray-100 text-gray-700">
                  ...
                  <td
                    className={`px-3 py-2 text-right font-medium ${
                      row.transact < 0 ? "text-rose-600" : "text-emerald-600"
                    }`}
                  >
